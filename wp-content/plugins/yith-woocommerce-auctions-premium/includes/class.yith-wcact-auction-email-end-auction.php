...
if ( ! class_exists( 'YITH_WCACT_Email_End_Auction' ) ) {
    ...
    class YITH_WCACT_Email_End_Auction extends WC_Email {
        ...
        public function __construct() {
            ...
            $this->title = __('​Auctions - The Auction is about to end', 'yith-auctions-for-woocommerce');
            $this->title = __('​Auctions - End', 'yith-auctions-for-woocommerce');

            $this->customer_email = true;

            // this is the description in WooCommerce email settings
            $this->description = __('​Can be emailed when the auction is about to end', 'yith-auctions-for-woocommerce');

            // these are the default heading and subject lines that can be overridden using the settings
            $this->heading = __('The Auction is about to end', 'yith-auctions-for-woocommerce');
            $this->subject =  __('The Auction is about to end', 'yith-auctions-for-woocommerce');
        ...
        public function trigger( $user_id, $product_id ) {
            ...
            if ( $unit == "days" ) {
                $unit = _x('days','Admin option: days','yith-auctions-for-woocommerce');
            } elseif ($unit =="hours"){
                $unit = _x('hours','Admin option: hours','yith-auctions-for-woocommerce');

            } else {
                $unit = _x('minutes','Admin option: hours','yith-auctions-for-woocommerce');
            }

            $this->object = array(
                'user_email' => ( isset( $user->data->user_email ) ) ? $user->data->user_email : $user_id,
                'user_name' => ( isset( $user->data->user_login ) ) ?  $user->data->user_login : $user_id,
