...
class WC_Product_Vendors_Store_Admin {
		...
		if ( 'wcpv_vendor_settings' === $current_section ) {
			$new_settings = array(
				...
				array(
					...
					'desc'     => __( 'Nu-ll-2-4.Net-Choose the frequency of commission payout for vendors.  Any commission that is unpaid will follow the schedule set here.  Note that by saving this option, a payout will initiate now and recur based on your settings from today\'s date.', 'woocommerce-product-vendors' ),
	...
	public function add_sold_by_order_item_detail( $item_id, $item, $product ) {
		...
		if ( 'yes' === $sold_by && ! empty( $item['product_id'] ) && WC_Product_Vendors_Utils::is_vendor_product( $item['product_id'] ) ) {
			...
			echo '<em class="wcpv-sold-by-order-details">' . apply_filters( 'wcpv_sold_by_text', __( 'จำหน่ายโดย:', 'woocommerce-product-vendors' ) ) . ' <a href="' . esc_url( $sold_by['link'] ) . '" title="' . esc_attr( $sold_by['name'] ) . '">' . $sold_by['name'] . '</a></em>';
		}
	}
