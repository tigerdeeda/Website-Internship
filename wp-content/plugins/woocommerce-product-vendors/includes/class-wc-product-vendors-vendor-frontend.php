...
class WC_Product_Vendors_Vendor_Frontend {
	...
	public function add_sold_by_loop() {
		...
		if ( 'yes' === $sold_by ) {
			...
			echo '<em class="wcpv-sold-by-loop">' . apply_filters( 'wcpv_sold_by_text', esc_html__( 'จำหน่ายโดย:', 'woocommerce-product-vendors' ) ) . ' <a href="' . esc_url( $sold_by['link'] ) . '" title="' . esc_attr( $sold_by['name'] ) . '">' . $sold_by['name'] . '</a></em>';
		}
	...
	public function add_sold_by_single() {
		...
		if ( 'yes' === $sold_by ) {
			...
			echo '<em class="wcpv-sold-by-single">' . apply_filters( 'wcpv_sold_by_text', esc_html__( 'จำหน่ายโดย:', 'woocommerce-product-vendors' ) ) . ' <a href="' . esc_url( $sold_by['link'] ) . '" title="' . esc_attr( $sold_by['name'] ) . '">' . $sold_by['name'] . '</a></em>';
		}
