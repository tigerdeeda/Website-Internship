...
if (count($auction_list) == 0){
    ...
    <table id="datatable">
        <tr>
            ...
            <td class="toptable"><?php echo __('ราคาประมูล','yith-auctions-for-woocommerce'); ?></td>
            <td class="toptable"><?php echo __('วันเวลา','yith-auctions-for-woocommerce'); ?></td>
        </tr>
        <?php
        ...
        if ($product->is_start() && $auction_list) {
            ?>
            <tr>
                <td><?php _e('เริ่มต้นประมูล', 'yith-auctions-for-woocommerce') ?></td>
...
<?php
    if (count($auction_list) == 0) {
        ...
        <p id="single-product-no-bid"><?php _e('ขณะนี่ยังไม่มีสินค้าสำหรับการประมูลอยู่ขณะนี่', 'yith-auctions-for-woocommerce'); ?></p>
